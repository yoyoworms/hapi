...
async function fetchLatestMessagesOnce(
    ...
    try {
        // Fork's incremental mode: when explicitly requested and we have a known cursor,
        // only fetch messages strictly newer than the local latest. Otherwise fall back
        // to upstream's cold-load backfill flow.
