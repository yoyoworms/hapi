...
export async function startRunner(options: { workspaceRoots?: string[] } = {}): Promise<void> {
  ...
  try {
    ...
    const spawnSessionOnce = async (options: SpawnSessionOptions): Promise<SpawnSessionResult> => {
      logger.debugLargeJson('[RUNNER RUN] Spawning session', { ...options, token: options.token ? '[REDACTED]' : undefined });
      ...
      try {
        ...
        // Resolve Codex account selection locally. Credentials and isolated
        // CODEX_HOME values never leave the runner.
        ...
        // Legacy explicit token injection remains supported.
        if (options.token) {
        ...
        if (options.sandbox) {
          ...
          logger.debug('[RUNNER RUN] Spawning sandboxed session', {
            containerName,
            agent,
            directory: spawnDirectory,
            environmentKeys: ['HAPI_API_URL', 'CLI_API_TOKEN', 'HAPI_HOME', ...Object.keys(extraEnv)]
          });
          ...
          if (happyProcess.pid) {
            (happyProcess as typeof happyProcess & { __sandboxContainer?: string }).__sandboxContainer = containerName;
          }
        } else {
      ...
      for (const [pid, session] of pidToTrackedSession.entries()) {
        if (session.happySessionId === sessionId ||
          ...
          if (session.startedBy === 'runner' && session.childProcess) {
            try {
              const containerName = (
                session.childProcess as typeof session.childProcess & { __sandboxContainer?: string }
              ).__sandboxContainer;
              if (containerName) {
