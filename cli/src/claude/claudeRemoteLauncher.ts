...
import { FileUploadTracker } from "./utils/fileUploader";
...
class ClaudeRemoteLauncher extends RemoteLauncherBase {
    ...
    protected async runMainLoop(): Promise<void> {
        ...
        // Register flush handler so Hub can request queue flush before storing user messages
        session.client.rpcHandlerManager.registerHandler('flushQueue', async () => {
        ...
        // Track and upload media files (images, etc.) to COS
        const cliApiToken = configuration.cliApiToken || process.env.CLI_API_TOKEN || '';
        const fileUploadTracker = new FileUploadTracker(cliApiToken);

        async function onMessage(message: SDKMessage) {
            // Capture rate limit events and forward as session events
            if (message.type === 'rate_limit_event' || message.type === 'rate_limit_info') {
                logger.info(`[remote]: rate limit data: ${JSON.stringify(message)}`);
            ...
            if (message.type === 'assistant') {
                ...
                if (umessage.message.content && Array.isArray(umessage.message.content)) {
                    ...
                    // Track Write tool calls for media files
                    fileUploadTracker.onAssistantMessage(umessage.message.content);
                }
            }
            ...
            if (logMessage) {
                ...
                // in Hub when the web/telegram client sent them. Only tool_result user
                // messages (generated by SDK after tool execution) should be forwarded.
                if (logMessage.type === 'user' && logMessage.message?.content) {
                    ...
                    if (!isToolResult) {
                        // Pure user text message — already in Hub, skip to avoid duplicates
                        return;
                    }
                ...
                // Upload media files to COS and attach URLs to tool_result content
                if (logMessage.type === 'user' && logMessage.message?.content && Array.isArray(logMessage.message.content)) {
                    const uploads = await fileUploadTracker.onToolResults(logMessage.message.content);
                    if (uploads.size > 0) {
                        for (let i = 0; i < logMessage.message.content.length; i++) {
                            const c = logMessage.message.content[i];
                            if (c.type === 'tool_result' && c.tool_use_id && uploads.has(c.tool_use_id)) {
                                logMessage.message.content[i] = {
                                    ...c,
                                    cosFileUrl: uploads.get(c.tool_use_id),
                                };
                            }
                        }
                    }
                ...
                try {
                    await claudeRemote({
                        ...
                        onReady: async (autonomous) => {
                            // A ready callback proves this attempt made progress.
